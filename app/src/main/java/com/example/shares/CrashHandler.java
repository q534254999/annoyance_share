...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    private static final String PATH = Environment.getExternalStorageDirectory().getPath() + "/SharesCrashInfo/log/";
    ...
    private void uploadExceptionToServer() {
        ...
        /*String DOMAIN = "http://" + "125.216.245.27:80";
        final String CRASHUPLOAD = "/Shares/crash/post";
        httpTask.url(DOMAIN + CRASHUPLOAD)
                .addParams("crash_file", new File(FILE_POSITION))
                .sendRequest();
                */
        if(!DEBUG){
            SharedPreferences.Editor editor=mContext.getSharedPreferences("log",Context.MODE_PRIVATE).edit();
            editor.putBoolean("hasLog",Boolean.TRUE);
            editor.putString("file",FILE_POSITION);
            editor.apply();
        }
